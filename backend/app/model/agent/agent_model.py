...
class Agent(Base):
    ...
    short_memory: Mapped[int] = mapped_column(default=0, comment='短期记忆(0关闭 1开启)')
    long_memory: Mapped[int] = mapped_column(default=0, comment='长期记忆(0关闭 1开启)')
    ...
    # Relationships
    interactions: Mapped[List['UserAgentInteraction']] = relationship("UserAgentInteraction", back_populates="agent",
                                                                      cascade="all, delete-orphan", init=False)
    use_users: Mapped[List['User']] = relationship('User', secondary=user_agent_map, back_populates='add_agents', init=False)
    ...
    publishments: Mapped[List['AgentPublishment']] = relationship(back_populates="agent",cascade="all, delete-orphan", init=False)
