...
class OperaLogMiddleware(BaseHTTPMiddleware):
    ...
    async def get_request_args(request: Request) -> dict:
        """获取请求参数，兼容 JSON、Form-data 和 XML"""
        ...
        # 2. 处理 XML 请求（如微信回调）
        elif body_data and ("text/xml" in content_type or "application/xml" in content_type):
            try:
                import xmltodict  # 推荐使用 xmltodict 解析 XML
                xml_dict = xmltodict.parse(body_data.decode("utf-8"))
                args.update({"xml_data": xml_dict})  # 将 XML 解析后的字典存入 args
            except Exception as e:
                args.update({"raw_xml": body_data.decode("utf-8")})  # 解析失败时存储原始 XML
        ...
        # 3. 处理 JSON 请求
            ...
            try:
                ...
                if not isinstance(json_data, dict):
                    json_data = {
                        f"{type(json_data).__name__}_data": (
                ...
                # 如果不是 JSON，尝试存储原始 body（可能是文本或其他格式）
                args.update({"raw_body": body_data.decode("utf-8")})
